...
processed_docs_as_dict = tp.fileToDict('antique_search_engine/files/processed-collection', 20000)
processed_documents = list(processed_docs_as_dict.values())
processed_documents_keys = list(processed_docs_as_dict.keys())


inverted_index = indexing.offlineReadIndex('antique_search_engine/files/index.json')
tfidf_matrix = tfidf.offlineReadMatrix('antique_search_engine/files/matrix.json')
...
def search(query):
   ...
   # get related document with cousine similarity
    ...
    result = []
    result_idxs = []
    for j in range(0, min (100000, len(result_with_idx) - 1)):
