...
processed_docs_as_dict = tp.fileToDict('lifestyle_forum_dev_search_engine/files/processed-collection2', 268892)
#processed_docs_as_dict = tp.fileToDict('lifestyle_forum_dev_search_engine/files/processed-collection3', 10000)
processed_documents = list(processed_docs_as_dict.values())
processed_documents_keys = list(processed_docs_as_dict.keys())


inverted_index = indexing.offlineReadIndex('lifestyle_forum_dev_search_engine/files/index2.json')
tfidf_matrix = tfidf.offlineReadMatrix('lifestyle_forum_dev_search_engine/files/matrix2.json')
...
print(search("ats are just like dogs in that they need a lot of exercise. More importantly, they also share the same instinct to kill, except to a much greater degre")[0])
